    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "4) reject 表示候选不应进入最终推荐；low 表示可作为弱相关补充但不能放在前面。",
            "5) group 只能从 core_gameplay、visual_support、follower_or_npc、requirement_or_patch、related_addon、other_related、off_topic 中选择。",
            "6) gaps 用来描述当前候选集缺少什么证据或类型，不是让你直接生成检索结果。",
            "",
            "JSON 结构：",
            "{",
            '  "judgements": [',
            '    {"candidate_id": 1, "relevance": "high|medium|low|reject", "group": "core_gameplay|visual_support|follower_or_npc|requirement_or_patch|related_addon|other_related|off_topic", "reason": "简短理由"}',
            "  ],",
