...
async def _run_llm_judge(tool_input: CandidateSemanticJudgeInput) -> CandidateSemanticJudgeResult | None:
    ...
    content = await client.chat(prompt, model=tool_input.model, max_tokens=1400, request_timeout=30.0)
    result = _result_from_json(json_object_from_text(content), raw_output=content)
    if result is not None:
        logger.info(
            "agent.tool name=candidate_semantic_judge status=succeeded candidates=%s judgements=%s evidence_id=%s",
            len(tool_input.candidates),
    ...
    if repaired is None:
        logger.info(
            "agent.tool name=candidate_semantic_judge status=fallback reason=invalid_repair_json evidence_id=%s",
            tool_input.evidence_id,
        )
        return None
    logger.info(
        "agent.tool name=candidate_semantic_judge status=succeeded source=repair candidates=%s judgements=%s evidence_id=%s",
        len(tool_input.candidates),
