...
- 本机现有 onedir EXE 已重新通过无 GUI packaged smoke；它是 Windows x64 PE，所需 WebView2/pythonnet DLL 齐全，bundle 禁入扫描通过。该二进制早于后续工作树改动，不是当前工作树的新鲜完整构建。
- 当前 `release` 目录不是可发布的最新完整资产集：只有 portable ZIP 与 SHA256，没有 Setup；ZIP 内 EXE 也不是当前 `dist-desktop` 中的同一构建。
- 当前 EXE 没有 Authenticode 签名。
...
因此，当前状态是「核心实现与本地自动化证据充分，发布和人工验收尚未完成」，不能宣称技术设计第 32 节的全部验收标准已经通过。
...
已验证（针对本机现有 `dist-desktop` 产物）：

- `dist-desktop\ModWatcherAgent\ModWatcherAgent.exe` 可运行 `--smoke-test`。
- smoke 启动真实进程内 FastAPI，检查 `/api/health` 与 `/`，创建隔离数据库和 `desktop.log`，随后确认进程、端口和临时目录清理。
- EXE 的 PE Machine 为 `0x8664`。
- WebView2 Core/WinForms、x64 Loader 与 `Python.Runtime.dll` 存在。
- onedir 禁入扫描没有发现运行时数据库、日志、profile、快照、缓存、测试或明显凭据。
...
当前本地 `release` 只包含 portable ZIP 与对应 SHA256，没有安装器及其 SHA256。portable 的摘要配对有效，但 ZIP 内 EXE 与当前 onedir EXE 不是同一构建。因此该目录不能作为当前源码的完整发布证据，也不能上传为最终 4 件资产。

本机已经校验并使用官方 Inno Setup 6.7.3：固定 tag attestation 成功，下载资产 SHA256 为 `9c73c3…b732`，Authenticode 状态为 `Valid`，签名者为 `Pyrsys B.V.`，并以 `/PORTABLE=1` 安装到临时目录。

首次真实编译暴露并随后修复了数组行 `Invalid section tag` 与 HKEY 类型错误。修复后使用最小 x64 源树验证两个预处理分支：

- 不带 WebView2：ISCC 退出码 0，Setup 为 2,116,878 bytes，SHA256 `bc6450…502b`。
- 带已验签官方 Evergreen Bootstrapper：Bootstrapper SHA256 `f91077…19c1`；ISCC 退出码 0，Setup 为 3,682,930 bytes，SHA256 `f9f4e4…2349`。

这些是 Inno 编译器来源和两个脚本分支的真实通过证据，但输入不是最终正式 onedir。Task 11 仍需从当前源码执行完整 onedir → installer → SHA256 构建并复验，安装、升级与卸载仍需人工矩阵。
...
| A05 | onedir 为首发形态 | spec 使用 `COLLECT`，portable 包含完整目录 | 自动通过 | 当前源码重建 R03 |
| A06 | 安装版与 portable | portable 逻辑和 Inno 脚本存在 | 部分通过 | Setup R02；portable R03 |
...
| B06 | Release 不含用户数据与秘密 | tree/ZIP/installer/workflow gates | 自动通过 | R03/R04 真实资产复验 |
...
| ID | 技术设计要求 | 当前证据 | 状态 | 剩余验收 |
|---|---|---|---|---|
| P01 | 打包前端、Alembic、别名、图标、README、LICENSE | spec 语义测试 | 自动通过 | R03 真实资产清单 |
...
| P04 | x64 PE 和 WebView2/pythonnet DLL | binary parser、required-file tests；本机 EXE | 本机产物通过 | R03 重建 |
| P05 | 清理/output/reparse/junction 边界 | packaging dynamic tests | 自动通过 | CI R03 |
| P06 | portable ZIP 与同名 SHA256 | portable behavior tests；旧本地 ZIP摘要匹配 | 部分通过 | 当前源码重建 R03 |
| P07 | 逐用户安装、开始菜单、可选桌面快捷方式 | Inno section tests | 静态通过 | M06、M14 |
...
| P10 | silent uninstall 保留数据 | `UninstallSilent` 早退 contract | 静态通过 | M14 |
| P11 | debug 构建开关/Debug EXE | 当前 `build_desktop.ps1` 没有 `-Debug` | 未执行 | 决定是否仍为首版要求 |
| P12 | Windows 代码签名 | 当前 EXE Authenticode 为 `NotSigned` | 未执行 | 发布签名策略 |
...
| C07 | 精确 4 件资产与 SHA256 双阶段复验 | 动态正/负 workflow tests | 自动通过 | R03、R04 |
...
| R03 | 当前源码完整构建 | onedir smoke、portable、Setup、两个 SHA256，精确 4 件且内容 clean | 未执行 |
...
| M05 | 中文用户名与含空格路径 | 安装、portable、数据、日志、迁移和 WebView 路径正常 | 未执行 |
| M06 | 非管理员逐用户安装 | 安装到 LocalAppData；开始菜单/可选桌面快捷方式正常 | 未执行 |
| M07 | WebView2 缺失 | 无 Bootstrapper时提示正确；有 Bootstrapper时成功/失败路径可操作 | 未执行 |
| M08 | 真实窗口和托盘 | 最小化/关闭隐藏、双击/菜单恢复、托盘故障降级 | 未执行 |
| M09 | 完整退出 | Scheduler/浏览器/后端/窗口/托盘关闭，进程和端口释放 | 未执行 |
...
| Release 泄露数据库/密钥 | 高 | 多阶段 tree/ZIP/installer/CI 禁入扫描 | 对最终 4 件资产再次复验 R03/R04 |
...
在这些条件完成前，文档只应使用「已实现」「自动测试通过」「本机 onedir smoke 通过」等有范围的表述，不应使用「Windows 10/11 已验收」「安装器已验证」或「GitHub Release 已成功发布」。
