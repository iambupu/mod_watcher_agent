...
- tag 版本一致性、显式 release notes、远端资产字节级比对和不使用 `--clobber` 的缺件恢复。清单查询失败、多余资产或任何已有资产不匹配时以失败状态退出且不上传。
...
| Task 9：发布 workflow | 供应链校验、质量门、精确 4 件资产、不覆盖发布与显式 release notes | workflow/publisher 动态正反测试、YAML/PowerShell/Bash 解析 | 静态通过 | GitHub-hosted 运行见 R01、R02、R04。 |
...
| A03 | 独立 EXE + React 静态前端 | spec 打包 `frontend/dist`；smoke 检查 HTML shell 及全部引用的本地脚本/样式 | 本机产物通过 | M01、M02、M08 |
...
| W05 | 自有后端 health 成功后才创建窗口 | controller readiness 与外来监听器拒绝测试 | 自动通过 | M08 |
...
| C08 | tag 创建或不覆盖恢复 GitHub Release | 远端清单/字节比对、缺件二阶段补传、多余/不匹配/failure 零上传动态测试 | 静态通过 | R04 |
...
| F09 | 冻结版开机启动写入带引号的绝对 EXE；源码模式保留 `start.ps1 -Tray` | autostart 精确命令测试；真实 HKCU 卸载归属删除/用户改写保留 | 部分通过 | 重新登录、设置关闭与 portable 移动后修正见 M16 |
...
| M16 | 冻结版开机启动 | 启用后 HKCU Run 指向当前绝对 EXE；重新登录可启动；关闭后删除；移动 portable 后重新启用可修正路径 | 部分通过：真实卸载删除精确归属 Run 值并保留用户改写值；登录启动、设置关闭与 portable 移动未执行 |
